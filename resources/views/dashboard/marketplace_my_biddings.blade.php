  ...
  <div class="mp-page">
    ...
    <div class="mp-toolbar" style="margin-bottom:24px; margin-top:12px;">
      <div>
        ...
        <p style="margin:4px 0 0; font-size:13px; color:#6b7280;">Track the status of your equipment requests.</p>
      </div>
    ...
    @else
      <div style="background:#fff; border-radius:16px; border:1px solid #e5e7eb; padding:12px 16px;">
        <div style="display:grid; grid-template-columns:2fr 1.5fr 1fr 1fr; gap:8px; padding:8px 0; font-size:12px; font-weight:600; color:#6b7280; border-bottom:1px solid #e5e7eb;">
          <div>Equipment</div>
          <div>Request ID</div>
          <div>Status</div>
          <div>Date</div>
        ...
        @foreach($biddings as $bid)
          ...
          @endphp
          <div style="display:grid; grid-template-columns:2fr 1.5fr 1fr 1fr; gap:8px; padding:10px 0; font-size:13px; align-items:center; border-bottom:1px solid #f3f4f6;">
            <div>
              <div style="font-weight:500; color:#111827;">{{ $product->name ?? 'Equipment Request' }}</div>
              <div style="margin-top:4px;">
                <a href="{{ route('marketplace.my_biddings.show', $bid) }}" style="font-size:12px; color:#2563eb; text-decoration:none;">
                  View details
                </a>
              </div>
            </div>
            <div>
              <span style="font-family:mono; font-size:12px;">{{ $bid->request_code }}</span>
            </div>
            <div>
            ...
            <div>
              <span style="font-size:12px; color:#4b5563;">{{ $bid->created_at->format('d M Y') }}</span>
            </div>
          </div>
        @endforeach
      </div>
